...
function displayMedication(meds) {
  med_list.innerHTML +=
    "<li id=med" +
    medCounter +
    " onclick=displayDosage(this);>" +
    meds +
    "</li>";
  medCounter++;
}

function displayDosage(med) {
  var medName =
    dosageInstructions[med.id.replace("med", "")].resource
      .medicationCodeableConcept.text;
  if (
    !dosageInstructions[med.id.replace("med", "")].resource.dosageInstruction
  ) {
    document.getElementById("medicineName").innerHTML =
      "No dosage instruction available.";
    document.getElementById("frequency").innerHTML = "";
    document.getElementById("period").innerHTML = "";
    document.getElementById("periodUnit").innerHTML = "";
    document.getElementById("suggestedTime").innerHTML = "";
  } else {
    document.getElementById("medicineName").innerHTML = medName;
  }
  var freq =
    dosageInstructions[med.id.replace("med", "")].resource.dosageInstruction[0]
  ...
  if (periodUnit === "d") {
    switch (freq) {
      case 1:
        ...
        break;
      case 2:
        suggestedTimes = "8AM, 6PM";
        break;
      case 3:
        suggestedTimes = "8AM, 12PM, 6PM";
        break;
    }
  }
...
client
  .request(`MedicationRequest?patient=${client.patient.id}`)
  .then(function (patient) {
    dosageInstructions = patient.entry;
  });
