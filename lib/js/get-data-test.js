...
const client = new FHIR.client({
  ...
  tokenResponse: {
    ...
    patient: "068973f7-ee9f-48ef-b4ec-fb6092b14302",
  },
});
...
client
  ...
  .then(function (ob) {
    ...
    // p.medication = getQuantityValueAndUnit(medication[0]);
...
client
  ...
  .then(function (patient) {
    var entries = patient.entry;
    entries.forEach(function (med) {
      medResults.push(med.resource.medicationCodeableConcept.text);
    });
...
client
  .request(`MedicationRequest?patient=${client.patient.id}`)
  .then(function (patient) {
    var medName = patient.entry[3].resource.medicationCodeableConcept.text;
    var freq =
      patient.entry[3].resource.dosageInstruction[0].timing.repeat.frequency;
    var period =
      patient.entry[3].resource.dosageInstruction[0].timing.repeat.period;
    var periodUnit =
      patient.entry[3].resource.dosageInstruction[0].timing.repeat.periodUnit;
    // var medText = patient.entry[0].resource.dosage[0].text;
    // var timingInfo = patient.entry[0].resource.dosage[0].timing.repeat;
    // var freq = timingInfo.frequency;
    // var period = timingInfo.period;
    // var periodUnit = timingInfo.periodUnit;
    var suggestedTimes = "";
    var currentTime = "";
    switch (freq) {
      case 1:
        suggestedTimes = "<button>12PM</button>";
        break;
      case 2:
        suggestedTimes = "8AM & 12PM";
        break;
      case 3:
        suggestedTimes =
          "1. <button disabled>8AM</button> Taken on 10/10/2020 8:11AM<br> 2. <button>12PM</button> 3. <button>6PM</button>";
        break;
    }

    document.getElementById("medicineName").innerHTML = medName;
    document.getElementById("frequency").innerHTML = freq;
    document.getElementById("period").innerHTML = period;
    document.getElementById("periodUnit").innerHTML = periodUnit;
    document.getElementById("suggestedTime").innerHTML = suggestedTimes;
  });
